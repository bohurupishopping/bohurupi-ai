...
export default function CompletedOrders() {
  ...
  const fetchCompletedOrders = useCallback(async () => {
    try {
      ...
      const ordersCollection = collection(db, 'orders') as CollectionReference<FirestoreOrder>;
      
      // Query for completed orders
      const q = query(
        ordersCollection,
        where('status', '==', 'completed'),  // Changed from 'pending' to 'completed'
      ...
      <div className="relative space-y-4 sm:space-y-6">
        <Breadcrumb className="z-10 bg-white/70 dark:bg-gray-800/70 backdrop-blur-md rounded-xl border border-gray-100/80 dark:border-gray-700/80 shadow-sm">
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>Completed Orders</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
